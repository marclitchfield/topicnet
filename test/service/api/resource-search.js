...
describe('Resource Search', function() {
	...
	before(function(done) {
		return api.post('/resources', { title: 'Resource to find! ' + guid.raw(),
	...
	describe('GET /resources?invalid', function() {
		...
		it('returns status 404', function() {
			return api.get('/resources?invalid')
			.then(function(res) {
	...
	describe('GET /resources?title with exact matching title', function() {
		...
		it('returns existing resource', function() {
			return api.get('/resources?title=' + resourceToFind.title)
			.then(function(res) {
	...
	describe('GET /resources?url with exact matching url', function() {
		...
		it('returns resource ', function() {
			return api.get('/resources?url=' + encodeURIComponent(resourceToFind.url))
			.then(function(res) {
				var searchResults = api.parseBody(res.body);
				assert.ok(_.all(searchResults, function(r) {
	...
	describe('GET /resources?q with partial title', function() {
		...
		it('returns a resource with the search string in the title', function() {
			return api.get('/resources?q=' + searchString)
			.then(function(res) {
				var searchResults = api.parseBody(res.body);
				assert.ok(_.all(searchResults, function(r) {
	...
	describe('GET /resources?q with query that will return over 10 results', function() {
		...
		describe('then GET /resources?q with the same query but p=1 and pp=5', function() {
			...
			it('returns the first 5 matching results from the previous set', function() {
				return api.get('/resources?q=' + searchString + '&p=1&pp=5')
				.then(function(res) {
					var searchResults = api.parseBody(res.body);
		...
		describe('then GET /resources?q with same the query but p=2 and pp=5', function() {
			...
			it('returns the last 5 matching results from the previous set', function() {
				return api.get('/resources?q=' + searchString + '&p=2&pp=5')
				.then(function(res) {
					var searchResults = api.parseBody(res.body);
					assert.equal(searchResults.length, 5);
					for(var i = 0; i < 5; i++) {
