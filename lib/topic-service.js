	...
	return {
		...
		get: function(id, success, fail) {
			...
			graph.query(cypherQuery, function(err, results) {
				if (results === undefined) {
					deferred.reject({name: 'notfound', message: 'topic with id ' + id + ' not found'});
				} else if (err) {
					deferred.reject(err);
				} else {
					deferred.resolve(helper.makeTopics(results)[0]);
				}
			});
		...
		*/
		search: function(query, success, fail) {
			...
			var deferred = Q.defer();
			graph.query(cypherQuery, params, deferred.makeNodeResolver());
		...
		create: function(topic, success, fail) {
			...
			checkForDuplicateNew(topic, failDuplicate, deferred.resolve);

			return deferred.promise.then(function() {
				var saveDeferred = Q.defer();
				var node = graph.createNode(topic);
				node.save(saveDeferred.makeNodeResolver());

				return saveDeferred.promise.then(function() {
					var updateIndexDeferred = Q.defer();
					updateTopicIndex(node, topic, updateIndexDeferred.makeNodeResolver());
					return updateIndexDeferred.promise;
				})
				.then(function() {
		...
		update: function(id, topic) {
			...
			checkForDuplicateUpdate(topic, id, failDuplicate, deferred.resolve);

			return deferred.promise.then(function() {
				var getDeferred = Q.defer();
				graph.getNodeById(id, getDeferred.makeNodeResolver());
			...
			.then(function(node) {
				...
				return saveDeferred.promise.then(function() {
					...
					updateTopicIndex(node, topic, updateDeferred.makeNodeResolver());
					return updateDeferred.promise;
				})
				.then(function() {
					helper.makeNode(node._data);
				});
			})
